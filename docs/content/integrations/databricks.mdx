...
databricks_client_instance = databricks_client.configured(
    {
        ...
        "legacy_api: False,  # This is required for backwards compatibility reasons until the `legacy_api` is removed.
    }
)
```
...
def my_databricks_op(context: OpExecutionContext) -> None:
    ...
    databricks_api_client.dbfs.read(path="/tmp/HelloWorld.txt")
...
def my_databricks_table(context: OpExecutionContext) -> None:
    ...
    databricks_api_client.jobs.run_now(job_id=DATABRICKS_JOB_ID)
